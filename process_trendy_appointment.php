...
function logError($message) {
    $logFile = 'appointment_errors.log';
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        // Recipient email
        $to = "trendy@streetngsl.com";
        
        // Set a proper From address that matches your domain
        $from_email = "appointments@streetngsl.com"; // Use your actual domain
        $from_name = "Street Trendy Appointments";
        
        // Email subject
        $email_subject = "New Appointment Request: " . $service;
        
        // Prepare email content
        $email_body = "You have received a new appointment request from the Street Trendy website.\n\n";
        ...
        try {
            ...
            $errors[] = "We couldn't send your appointment request. Please try again later or contact us directly by phone.";
        }
    }
}
...
function sendConfirmationEmail($to_email, $to_name, $service) {
    ...
    $subject = "Your Appointment Request: $service";
    
    $message = "Dear $to_name,\n\n";
    $message .= "Thank you for scheduling an appointment with Street Trendy for $service.\n\n";
    $message .= "We have received your request and will confirm your appointment shortly.\n\n";
    $message .= "If you need to make any changes to your appointment, please contact us at:\n";
    $message .= "Phone: +2347086067631\n";
    $message .= "Email: trendy@streetngsl.com\n\n";
    $message .= "Best regards,\n";
    $message .= "Street Trendy Team\n";
...
function saveToDatabase($name, $email, $phone, $service, $message) {
    ...
    // Prepare and bind
    $stmt = $conn->prepare("INSERT INTO trendy_appointments (name, email, phone, service, message, submission_date) VALUES (?, ?, ?, ?, ?, NOW())");
...
// Redirect back to trendy page with status
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if ($success) {
        header("Location: trendy.html?status=success#contact");
        exit;
    } else {
        ...
        header("Location: trendy.html?status=error#contact");
        exit;
    }
}
?>